...
app.include_router(registration_router, prefix="/api")  # Node registration (2 endpoints)

# Total: 79 endpoints across 10 modular routers
...
async def startup_event():
    ...
    async def broadcast_cached_news_startup():
        try:
            ...
            await asyncio.sleep(5)  # Attendre 5s que le WebSocket soit prêt
            print("📢 Diffusion des news en cache...")
            await broadcast_cached_news()
            print("✅ News cache broadcast completed")
        except Exception as e:
    ...
    try:
        while True:
            ...
            if message["type"] == "test_connection":
                ...
                # Use the public IP sent by the frontend if available
                frontend_public_ip = message.get("client_public_ip")
                final_client_ip = frontend_public_ip or client_ip

                print(f"🌐 CLIENT_CONNECTED message received")
                print(f"   WebSocket IP: {client_ip}")
                print(f"   Frontend public IP: {frontend_public_ip}")
                print(f"   Using IP: {final_client_ip}")

                try:
                    from fededge_node_client import FedEdgeNodeClient
                    node_client = FedEdgeNodeClient()
                    print(f"📤 Calling start_session with client_ip={final_client_ip}")
                    result = node_client.start_session(client_ip=final_client_ip)
                    print(f"✅ Client session updated with IP: {final_client_ip}, result: {result}")
                except Exception as e:
