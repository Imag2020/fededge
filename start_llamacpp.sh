...
# Détecte automatiquement si un GPU NVIDIA est disponible
# et ajoute les flags appropriés pour llama.cpp
GPU_LAYERS="${GPU_LAYERS:-auto}"  # auto, 0 (CPU only), ou nombre de couches

detect_gpu() {
  # Si l'utilisateur a forcé le mode CPU
  ...
  if command -v nvidia-smi >/dev/null 2>&1; then
    if nvidia-smi >/dev/null 2>&1; then
      # GPU détecté, charger toutes les couches sur GPU
      info "🎮 GPU NVIDIA détecté, activation GPU (toutes couches)"
      echo "-ngl 99"
      return
    fi
  fi
  ...
  info "💻 Pas de GPU détecté, utilisation CPU/BLAS"
  echo ""
}
